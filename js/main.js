...
$(function(){
	...
	countDown.prototype={
		_renderText:function(){
			var self=this,nowsec=self.count;
			var seconds=nowsec%60+"",minutes=parseInt(nowsec/60)+"";
	...
	function _music(conf){
		...
		this.src=conf.src||"images/bg.mp3";
		this.dom=$(_this.id);
		this.init=function(){
	...
	function addOri(){
		window.ondeviceorientation =  function(e) {
			...
			if($(".last-person").length>=1)
			{
				...
				ang=ang*10;
				ang<-$(window).width()/2?ang=-$(window).width()/2:"";
				ang>$(window).width()/2?ang=$(window).width()/2:"";
	...
	function init(){
		...
		window.unlockm=new _music({
			id:"#unlock",
			src:"images/unlock.wav"
		});
		window.ringm=new _music({
			id:"#ring",
			src:"images/phone.mp3"
		});
